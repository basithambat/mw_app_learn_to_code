...
class AddExecutorScreen extends StatefulWidget {
  final String? willId;

  const AddExecutorScreen({Key? key, this.willId}) : super(key: key);

  @override
  State<AddExecutorScreen> createState() => _AddExecutorScreenState();
}

class _AddExecutorScreenState extends State<AddExecutorScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      // Assign as executor
      await _arrangementsService.assignExecutor(widget.willId!, person['id']);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to add executor: ${e.toString()}')),
        );
      }
    } finally {
      if (mounted) {
